						...
						resp = JSON.parse(resp)
							if(Object.keys(resp).length > 0){
								...
								Object.keys(resp).map(k=>{
									...
									// Admins pov for the price of passenger price
									tr.append('<td class="text-center">Rs '+resp[k].amount+'</td>')
									tr.append('<td class="text-center">'+(resp[k].status == 0 ? 'Paid' :'Unpaid')+'</td>')
